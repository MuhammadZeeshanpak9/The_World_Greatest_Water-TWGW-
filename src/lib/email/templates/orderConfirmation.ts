  ...
  const body = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:24px;">
      ...
      ${
        shippingCost != null
          ? `<tr>
        <td style="padding:10px 0 0;font-family:Arial,sans-serif;font-size:13px;color:#8e8e9e;">Subtotal</td>
        <td></td>
        <td style="padding:10px 0 0;font-family:Arial,sans-serif;font-size:13px;color:#4a4a5a;text-align:right;">$${itemsSubtotal.toFixed(2)}</td>
      </tr>
      <tr>
        <td style="padding:4px 0 0;font-family:Arial,sans-serif;font-size:13px;color:#8e8e9e;">Shipping${shippingCarrier ? ` (${escapeHtml(shippingCarrier)}${shippingService ? ` — ${escapeHtml(shippingService)}` : ""})` : ""}</td>
        <td></td>
        <td style="padding:4px 0 0;font-family:Arial,sans-serif;font-size:13px;color:#4a4a5a;text-align:right;">${shippingCost === 0 ? "Free" : `$${shippingCost.toFixed(2)}`}</td>
      </tr>`
          : ""
      }
      <tr>
    ...
    ${
      trackingNumber
        ? `<h2 style="font-family:Arial,sans-serif;font-size:12px;letter-spacing:1px;text-transform:uppercase;color:#8e8e9e;margin:0 0 8px;">Tracking</h2>
    <p style="font-family:Arial,sans-serif;font-size:14px;line-height:1.6;color:#4a4a5a;margin:0 0 24px;">
      ${escapeHtml(trackingNumber)}${trackingUrl ? ` — <a href="${trackingUrl}" style="color:${BRAND_VIOLET};">Track Package</a>` : ""}
    </p>`
        : ""
    }
